...
import (
	...
	"github.com/amitabhprasad/bookstore-app/bookstore-users-api/src/domain/users"
	"github.com/amitabhprasad/bookstore-app/bookstore-users-api/src/services"
...
func GetUser(c *gin.Context) {
	if err := oauth2.AuthenticateRequest(c.Request, auth_url); err != nil {
		c.JSON(err.Status(), err)
		return
	}

	if callerId := oauth2.GetCallerId(c.Request); callerId == 0 {
		err := rest_errors.NewRestError("Resource not available please check accessToken passed",
			http.StatusUnauthorized, "authId not found", nil)
		c.JSON(err.Status(), err)
		return
	}
